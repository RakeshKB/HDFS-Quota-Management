...
class UserYodaMetrics(object):
    ...
    def __init__(self):
        ...
        self.USER_YODA_COL_NAME = settings.INFO['USER_YODA_COL_NAME']
    ...
    def run(self):
        ...
        """
        try:
            self.TOTAL_ALLOCATED_SPACE , self.TOTAL_USED_SPACE, self.TOTAL_REMAINING_SPACE = self.get_userquota_details()
    ...
    def get_space_quota_query(self):
        space_quota_query = "scale(prod.uh1.grid.UH1_GOLD.app.glgw4009.quota.user.yoda.SPACE_QUOTA,1)\
&from=%s&until=%s&format=json" % (self.FROM_TIME, self.TO_TIME)
        query = settings.INFO['HDFS_USAGE_METRICS_BASE_QUERY'] + space_quota_query
        return query

    def get_remaining_space_quota_query(self):
        remaining_space_quota_query = "scale(prod.uh1.grid.UH1_GOLD.app.glgw4009.quota.user.yoda.REMAINING_SPACE_QUOTA,1)\
&from=%s&until=%s&format=json" % (self.FROM_TIME, self.TO_TIME)
        query = settings.INFO['HDFS_USAGE_METRICS_BASE_QUERY'] + remaining_space_quota_query
        return query

    def get_userquota_details(self):
        ...
        if self.TOTAL_ALLOCATED_SPACE  == None:
            ...
            self.TOTAL_ALLOCATED_SPACE  = round((self.TOTAL_ALLOCATED_SPACE /(1024**5)),2)

        if self.TOTAL_REMAINING_SPACE == None:
            self.TOTAL_REMAINING_SPACE = 0.0
        else:
            self.TOTAL_REMAINING_SPACE = round((self.TOTAL_REMAINING_SPACE /(1024**5)),2)

        self.TOTAL_USED_SPACE = round((self.TOTAL_ALLOCATED_SPACE  - self.TOTAL_REMAINING_SPACE),2)

        print "Total Allocated Space is %s PiB" % (self.TOTAL_ALLOCATED_SPACE)
        print "Total Used Space is %s PiB" % (self.TOTAL_USED_SPACE)
        print "Total Remaining Space is %s PiB" % (self.TOTAL_REMAINING_SPACE)
    ...
    def update_data_to_mongodb(self, total_allocated_space, total_used_space, total_remaining_space):
        ...
        """
        collection_name = self.USER_YODA_COL_NAME
        collection = get_connected.connect_to_mongodb(collection_name)
        if collection:
            collection.update({'date': self.CURRENT_DATE}, {'date': self.CURRENT_DATE, 'total_allocated_space_PiB':total_allocated_space, 'total_used_space_PiB': total_used_space, 'total_remaining_space_PiB':total_remaining_space, 'epoch_time': self.EPOCH_TIME }, upsert=True)
