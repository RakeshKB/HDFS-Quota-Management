...
class UserPlatformMetrics(object):
    """
    ...
    def __init__(self):
        ...
        self.USER_PLATFORM_COL_NAME = settings.INFO['USER_PLATFORM_COL_NAME']
    ...
    def run(self):
        ...
        """
        try:
            self.TOTAL_ALLOCATED_SPACE , self.TOTAL_USED_SPACE, self.TOTAL_REMAINING_SPACE = self.get_userplatform_details()
    ...
    def get_space_quota_query(self):
        space_quota_query = "scale(prod.uh1.grid.UH1_GOLD.app.glgw4009.quota.projects.userplatform.\
SPACE_QUOTA,1)\&from=%s&until=%s&format=json" % (self.FROM_TIME, self.TO_TIME)
    ...
    def get_remaining_space_quota_query(self):
        remaining_space_quota_query = "scale(prod.uh1.grid.UH1_GOLD.app.glgw4009.quota.projects.userplatform.\
REMAINING_SPACE_QUOTA,1)\&from=%s&until=%s&format=json" % (self.FROM_TIME, self.TO_TIME)
    ...
    def get_userplatform_details(self):
    ...
    def update_data_to_mongodb(self, total_allocated_space, total_used_space, total_remaining_space):
        ...
        """
        collection_name = self.USER_PLATFORM_COL_NAME
